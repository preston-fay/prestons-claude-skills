...
description: End-to-end video production skill that interviews users, collects brand assets, composes scenes, and generates complete Remotion projects — no video editing experience required. Use this skill whenever the user wants to create a video, motion graphics, animated explainer, product demo, social media clip, presentation video, promotional video, or any programmatic video content. Also trigger when users mention Remotion, video from code, animated content, video templates, or video generation. This skill handles the ENTIRE pipeline: creative interview → scene composition → Remotion code generation → quality review. Even if the user only says "make me a video" or "I need a promo clip," this skill guides them through every decision they need to make.
---
